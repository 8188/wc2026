<script lang="ts">
	...
	import {
		...
		placeBet, settleBet, cancelBet, resetBetting,
	...
	let betType = $state<'1x2' | 'spread' | 'over_under'>('1x2');
	...
	let currentOdds = $derived.by(() => {
		...
		if (betType === 'over_under') {
			return selection === 'over' ? (bk.over25 || 0) : (bk.under25 || 0);
		}
	...
	// Get the actual spread line from bookmaker data
	let actualSpreadLine = $derived.by(() => {
		if (!selectedPred) return null;
		const bkEntries = Object.values(selectedPred.bookmakerOdds || {});
		if (bkEntries.length === 0) return null;
		return bkEntries[0]?.spread?.line ?? null;
	});
	...
	async function handlePlaceBet() {
		...
		try {
			const result = await placeBet(
				...
				stake,
				betType,
				betType === 'spread' ? actualSpreadLine : null,
				betType === 'over_under' ? 2.5 : null,
				getBookmakerName()
			);
	...
	function handleSettle(betId: number, result: 'won' | 'lost') {
		return async () => {
			try {
	...
	async function handleCancel(betId: number) {
		try {
			const r = await cancelBet(betId);
			pendingBets = pendingBets.filter((b) => b.id !== betId);
			stats.bankroll = r.bankroll;
			message = '↩️ 已取消下注，注金已退回';
		} catch (e) {
			message = `❌ ${(e as Error).message}`;
		}
		setTimeout(() => (message = ''), 5000);
	...
	function betTypeLabel(t: string) {
		const map: Record<string, string> = { '1x2': '胜平负', spread: '让球盘', over_under: '大小球' };
		return map[t] || t;
	}
...
<div class="betting-page">
	...
	<div class="section">
		...
		<div class="fg">
			...
			{:else if betType === 'spread'}
				<div class="spread-row">
					{#if actualSpreadLine != null}
						<span class="spread-label">让球线: {actualSpreadLine > 0 ? '+' : ''}{actualSpreadLine}</span>
					{:else}
						<span class="spread-label">暂无让球盘数据</span>
					{/if}
					<div class="sel-btns">
			...
			{:else if betType === 'over_under'}
				<div class="spread-row">
					<span class="spread-label">盘口: 2.5 球</span>
					<div class="sel-btns">
		...
		{#if selectedPred}
			<div class="bet-preview">
				...
				<div class="pv-info">
					...
					{#if betType === 'spread' && actualSpreadLine != null} | 让球线: {actualSpreadLine > 0 ? '+' : ''}{actualSpreadLine}{/if}
					{#if betType === 'over_under'} | 盘口: 2.5球{/if}
				</div>
				<div class="pv-odds">
		...
		{/if}
		<button class="place-btn" disabled={!selectedPred || currentOdds <= 0} onclick={handlePlaceBet}>
			{#if !selectedPred}请先选择比赛{:else if currentOdds <= 0}暂无赔率，无法下注{:else}🎯 确认下注{/if}
		</button>
	</div>
			...
			{#each pendingBets as bet (bet.id)}
				<div class="bet-item">
					...
					<div class="ba">
						...
						<button class="cancel-b" onclick={() => handleCancel(bet.id)}>↩️ 取消</button>
					</div>
				</div>
			{/each}
		{/if}
		...
		{:else}
			{#each history as h}
				<div class="history-item">
					<span class="hr {(h as any).result}">{(h as any).result === 'won' ? '✅' : (h as any).result === 'cancelled' ? '↩️' : '❌'}</span>
					<div class="hi">
					...
					<div class="hp" style="color:{profitColor((h as any).profit)}">
						{#if (h as any).result === 'cancelled'}已取消{:else}{(h as any).profit > 0 ? '+' : ''}{(h as any).profit}{/if}
					</div>
				</div>
			{/each}
		{/if}
...
<style>
	...
	.spread-label { font-size: 13px; color: #ffd54f; font-weight: bold; white-space: nowrap; }
	...
	.place-btn:hover:not(:disabled) { background: linear-gradient(135deg, #388e3c, #2e7d32); }
	.place-btn:disabled { background: #2a3040; color: #556; cursor: not-allowed; opacity: 0.7; }
	...
	.cancel-b { background: #2a3040; border: 1px solid #3a4050; color: #aaa; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 12px; }
	.cancel-b:hover { background: #3a4050; color: #fff; }
	...
	.hr.cancelled { background: #2a3040; color: #aaa; }
	.hi { flex: 1; }
